import { Component, OnInit } from '@angular/core';
import { summaryFileName } from '../../../node_modules/@angular/compiler/src/aot/util';

@Component({
  selector: 'app-conflict-form',
  templateUrl: './conflict-form.component.html',
  styleUrls: ['./conflict-form.component.css']
})
export class ConflictFormComponent implements OnInit {

  constructor() { }
  headerDate;
  imgSource;
  doctorName;
  doctorDesignation;
  doctorPhone;
  doctorEMail;
  requestSubject;
  requestBody;
  fromDate;
  toDate;
  fromTime;
  toTime;
  effectedClinics;
  conflictCounts;
  totalConflicts;
  clinicColors;
  ngOnInit() {
    this.headerDate = 'March 7, 2018';
    // tslint:disable-next-line:max-line-length
    this.imgSource = 'data:image/png;base64,';
    this.doctorName = 'Peter Parker';
    this.doctorDesignation = 'Lung Cancer Diagnosis';
    this.doctorPhone = '+42-xxx-xxxxxxx';
    this.doctorEMail = 'peter.parker@outlook.com';
    this.requestSubject = 'Sick Leave';
    this.requestBody = 'Anim pariatur cliche reprehenderit, enim eiusmod high life accusamus terry richardson ad squid. Nihil anim keffiyeh helvetica, enim eiusmod high life accusamus terry terry richardson ad squid. Nihil animiyeh helv richardson craft beer labore wes anderson cred nesciunt sapiente ea proident.';
    this.fromDate = 'March 10, 2018';
    this.fromTime = '9:00 AM';
    this.toDate = 'March 13, 2018';
    this.toTime = '10:00 AM';
    this.effectedClinics = ['Gun Hill', 'Jamiaca', 'West Hill' , 'Bella'];
    this.conflictCounts = [5, 2, 4, 3];
    this.totalConflicts = 0;
    for (let i = 0 ; i < this.conflictCounts.length; i++) {
      this.totalConflicts += this.conflictCounts[i];
    }
    this.clinicColors = ['yellowgreen', 'orangered', 'royalblue', 'darkred', 'orange'];
}

}
